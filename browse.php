...
<div id="ResultsArea" class="Browse" style="margin-bottom: 25%">
    <?php
    if (isset($ReturnableResult) && count($ReturnableResult) > 0) {
        foreach ($ReturnableResult as $row) {
            ?>
            <a class="ListItem" href='view.php?id=<?php print $row['StockItemID']; ?>' id="itemRow<?php print $row['StockItemID'];?>">
                <div id="ProductFrame">
                    <?php
                    ...
                    <div id="StockItemFrameRight">
                        <div class="CenterPriceLeftChild">
                            <h1 class="StockItemPriceText"><?php print "€ ".number_format(sprintf(" %0.2f", berekenVerkoopPrijs($row["RecommendedRetailPrice"], $row["TaxRate"])),2,",","."); ?></h1>
                            <h6>Inclusief BTW </h6>
                            <form method="post" action="#itemRow<?php print $row['StockItemID'];?>">
                                <input type="submit" name="toevoegen<?php print $row['StockItemID'];?>" style="background-color: #ffc600; border-radius: 7px; border-color: black;text-align: center; text-decoration: none; color: black"  value="Toevoegen Aan Winkelmand">
                            </form>
                            <?php if(isset($_POST['toevoegen'.$row['StockItemID']])){
                                $cart=getCart();
                                if(array_key_exists($row['StockItemID'],$cart)){
                                    $newAmount = $cart[$row['StockItemID']]+1;
                                }else{
                                    $newAmount = 1;
                                }
                                addProductToCart($row["StockItemID"],$row['QuantityOnHand'],$newAmount,true);
                            }?>
                        </div>
                    </div>
